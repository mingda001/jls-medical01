...
<body>
<s:form action="saveBizrecord" method="post" theme="simple">
	<s:hidden name="businessDTO.ssn"></s:hidden>
	<s:hidden name="businessDTO.familyno"></s:hidden>
	<s:hidden name="businessDTO.memberId"></s:hidden>
	<table align="center" width="90%" class="t1" border="0" cellpadding="0"
		cellspacing="0">
		<tr>
			<th colspan="4">医后报销就诊信息</th>
		</tr>
		<tr>
			<td width="90">就诊医院</td>
			<td><s:textfield name="businessDTO.hospital"></s:textfield></td>
			<td width="90">就医时间</td>
		...
		<tr>
			<td width="90">疾病诊断</td>
			<td><s:textfield name="businessDTO.sickname" /></td>
			<td width="90">参保类型</td>
		...
		<tr>
			<td width="90">业务类型</td>
			<td colspan="3"><s:select name="businessDTO.bizType"
		...
		<tr>
			...
			<td><s:textfield name="businessDTO.allmoney" value="0" />元</td>
			<td width="90">保障金额</td>
...
<script type="text/javascript">
	function countdays() {
		...
		document.getElementById('saveBizrecord_businessDTO_days').value = DateDiff(
				end1, begin1);
	}
	//计算天数差的函数，通用
