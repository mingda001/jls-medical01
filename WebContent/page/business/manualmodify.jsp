...
<s:form enctype="multipart/form-data" action="savemanual" method="post"
	...
	<s:hidden name="manual.manualId"></s:hidden>
	<s:hidden name="manual.manualno"></s:hidden>
	<input type="hidden" id="outflag"
		value="<s:property value="manual.outFlag"/>">
	<table align="center" width="100%" class="t1" border="0" cellpadding="0"
		...
		<tr>
			<td width="90"><script type="text/javascript">
				...
				function disp1(){
					var flag =savemanual.outflag.value;
					var ch1=false;
					if(flag=="0"){
					}else{
						ch1=true;
						}
					savemanual.savemanual_manual_outFlag.checked=ch1;
					var ch2= savemanual.savemanual_manual_inHospitalname;
					if(ch1){
						ch2.style.display="block";
						sss.style.display="block";
	...
	<table align="center" width="100%" class="t1" border="0" cellpadding="0"
		...
		<tr>
			...
			<td id="end"><input type="text" readonly="readonly" id="endtime"
				name="manual.endtime"
				value="<s:date name="manual.endtime" format="yyyy-MM-dd"/>" /></td>
			<td width="120">个人账户支付</td>
			<td><s:textfield name="manual.accountpayment"
				cssStyle="width:80" />元</td>
		</tr>
		<tr>
			<td>住院天数</td>
			<td><s:textfield readonly="true" name="manual.days"
				cssStyle="width:80" onblur="countdays()" onfocus="countdays()" />天</td>
			<td width="90">&nbsp;</td>
			<td>&nbsp;</td>
			<td width="120">统筹基金支付</td>
			<td><s:textfield onblur="count()" onchange="count()"
				name="manual.planpayment" cssStyle="width:80" />元</td>
		</tr>
		<tr>
			<td>预交住院押金</td>
			<td><s:textfield name="manual.predeposit" cssStyle="width:80" />元</td>
			<td width="90">退补住院押金</td>
			<td><s:textfield name="manual.recdeposit" cssStyle="width:80" />元</td>
		...
		<tr>
			<td>起付线</td>
			<td><s:textfield name="manual.initline" cssStyle="width:80" /></td>
			<td width="90">自付比例</td>
			<td><s:textfield name="manual.selfscale" cssStyle="width:80" /></td>
		...
		<tr>
			...
			<td><s:textfield name="manual.selfall" cssStyle="width:80"
				onblur="count()" onchange="count()" />元</td>
			<td width="120">保健对象补助支付</td>
			<td><s:textfield name="manual.healthpayment" cssStyle="width:80" />元</td>
		</tr>
		<tr>
			<td>住院费用总额</td>
			<td><s:textfield onblur="count()" onchange="count()"
				name="manual.hospitalmoney" cssStyle="width:80" />元</td>
			<td>原个人账户余额</td>
			<td><s:textfield name="manual.oaccountbalance"
				cssStyle="width:80" />元</td>
			<td width="120">大额商业保险支付</td>
			<td><s:textfield onblur="count()" onchange="count()"
				name="manual.bigpayment" cssStyle="width:80" />元</td>
		</tr>
		<tr>
			<td>个人账户余额</td>
			...
			<td><s:textfield name="manual.restpayment" cssStyle="width:80" />元</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			...
			<td><s:textfield name="manual.injurypayment" cssStyle="width:80" />元</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			...
			<td><s:textfield name="manual.bearpayment" cssStyle="width:80" />元</td>
		</tr>
		<tr>
