...
<title>各年度救助人数及资金统计表</title>
</head>
<body onLoad="createChart()">
<s:form action="distributionbyyear" method="post" theme="simple">选择地区：<s:select
...
<table id="stattable" width="99%" cellspacing="0" cellpadding="0"
	...
	<tr>
		...
		<th height="24">救助预算资金</th>
		<th height="24">救助实际使用资金</th>
		<th height="24">门诊所占比例</th>
		<th height="24">住院所占比例</th>
	</tr>
...
<script type="text/javascript">
	...
	function createChart1() {
		...
		if (l > 0) {
			for ( var i = 0; i < l; i++) {
				var v1 = document.getElementById("r" + i + "d3").innerText;
				var v2 = document.getElementById("r" + i + "d0").innerText;
				var v3 = document.getElementById("r" + i + "d2").innerText;
				xml = xml + "<col>";
				xml = xml + "<v1>" + v1 + "</v1><v2>救助金额</v2><v3>"+v2+"</v3>";
				xml = xml + "</col>";
				xml = xml + "<col>";
				xml = xml + "<v1>" + v3 + "</v1><v2>救助预算金</v2><v3>"+v2+"</v3>";
				xml = xml + "</col>";
			}
			xml = "<root>" + xml + "</root>";
		}
		document.getElementById("img001").src = 'distributionbyyearchart.action?xml='
				+ xml + '&title=' + title;
	}
</script>
</html>